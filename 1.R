...
fun70 <- read.csv("fundamental70.csv")
fun70$cusip <- substr(fun70$cusip,1,8)
prc70 <- read.csv("price70.csv")

fun70$intersect = ifelse(fun70$cusip %in% prc70$CUSIP,'yes','no')
fun70 <- fun70[!(fun70$intersect == "no"),]

prc70$intersect = ifelse(prc70$CUSIP %in% fun70$cusip, 'yes','no')
prc70 <- prc70[!(prc70$intersect == "no"),]
...
ret70 <- as.data.frame(cbind(prc70$date,prc70$CUSIP,prc70$ALTPRC))
names(ret70) <- c("Date","CUSIP","ALTPRC")
ret70$Date <- as.Date(as.character(prc70$date), format = "%Y%m%d")
ret70$ALTPRC <- as.numeric(ret70$ALTPRC)
ret70 <- ret70 %>% spread(CUSIP,ALTPRC) 
ret70 <- ret70 %>% xts(x = ret70[,-1], order.by = ret70$Date)
ret70 <- Return.calculate(ret70)

prc70$mktcap <- abs(prc70$ALTPRC) * prc70$SHROUT
mktcap70 <- as.data.frame(cbind(prc70$date, prc70$CUSIP, prc70$mktcap))
names(mktcap70) <- c("DATE", "CUSIP", "mktcap")
mktcap70$DATE <- as.Date(as.character(mktcap70$DATE), format = '%Y%m%d')
mktcap70$mktcap <- as.numeric(mktcap70$mktcap)
mktcap70 <- mktcap70 %>% spread(CUSIP, mktcap)
mktcap70 <- xts(x = mktcap70[, -1], order.by = mktcap70$DATE)
...
NCAV70 <- as.data.frame(cbind(fun70$datadate,fun70$cusip,fun70$act-fun70$lt))
names(NCAV70) <- c("Date","CUSIP","NCAV")
NCAV70$Date <- as.Date(as.character(NCAV70$Date), format = "%Y%m%d")
NCAV70$NCAV <- as.numeric(NCAV70$NCAV)
NCAV70 <- NCAV70[!duplicated(NCAV70[,c("Date","CUSIP")]),]
NCAV70 <- NCAV70 %>% spread(CUSIP,NCAV)
NCAV70 <- NCAV70 %>% xts(x = NCAV70[,-1], order.by = NCAV70$Date)
NCAV70 <- NCAV70[endpoints(NCAV70, on = 'years'),]
NCAV70 <- NCAV70[-72,]

NetIc70 <- as.data.frame(cbind(fun70$datadate,fun70$cusip,fun70$ni))
names(NetIc70) <- c("Date","CUSIP","Net Income")
NetIc70$Date <- as.Date(as.character(NetIc70$Date), format = "%Y%m%d")
NetIc70$`Net Income` <- as.numeric(NetIc70$`Net Income`)
NetIc70 <- NetIc70[!duplicated(NetIc70[,c("Date","CUSIP")]),]
NetIc70 <- NetIc70 %>% spread(CUSIP,`Net Income`)
NetIc70 <- NetIc70 %>% xts(x = NetIc70[,-1], order.by = NetIc70$Date)
NetIc70 <- NetIc70[endpoints(NetIc70, on = 'years'),]
NetIc70 <- NetIc70[-72,]

GPA70 <- as.data.frame(cbind(fun70$datadate,fun70$cusip,(fun70$revt-fun70$cogs)/fun70$at))
names(GPA70) <- c("Date","CUSIP","GP/A")
GPA70$Date <- as.Date(as.character(GPA70$Date), format = "%Y%m%d")
GPA70$`GP/A` <- as.numeric(GPA70$`GP/A`)
GPA70 <- GPA70[!duplicated(GPA70[,c("Date","CUSIP")]),]
GPA70 <- GPA70 %>% spread(CUSIP,`GP/A`)
GPA70 <- GPA70 %>% xts(x = GPA70[,-1], order.by = GPA70$Date)
GPA70 <- GPA70[endpoints(GPA70, on = 'years'),]
GPA70 <- GPA70[-72,]

Date70 <- as.data.frame(fun70$datadate)
names(Date70) <- "Date"
Date70$Date <- as.Date(as.character(Date70$Date), format = "%Y%m%d")
Date70 <- Date70[!duplicated(Date70$Date),]
Date70 <- as.data.frame(Date70)
Date70 <- Date70 %>% xts(x = Date70, order.by = Date70$Date)
Date70 <- Date70[.indexmon(Date70) == 11]
...
mktcap70_jun <- mktcap70[.indexmon(mktcap70) == 5]
mktcap70_dec <- mktcap70[.indexmon(mktcap70) == 11]

NCAV70_r <- as.data.frame(coredata(NCAV70)/coredata(mktcap70_dec))
NCAV70_r <- NCAV70_r %>% xts(x = NCAV70_r, order.by = index(Date70))

NetIc70_r <- as.data.frame(coredata(NetIc70)/coredata(mktcap70_dec))
NetIc70_r <- NetIc70_r %>% xts(x = NetIc70_r, order.by = index(Date70))
...
ret70 <- xts(x = t(winsor(t(ret70), trim = 0.005, na.rm =TRUE)), order.by = index(ret70))
...
NCAV70 <- xts(x = NCAV70, order.by = index(mktcap70_jun))
NetIc70 <- xts(x = NetIc70, order.by = index(mktcap70_jun))
GPA70 <- xts(x = GPA70, order.by = index(mktcap70_jun))
...
NCAV70_q <- as.data.frame(1:(length(endpoints(NCAV70_r, on = 'months'))-1))
for (i in 1:(length(endpoints(NCAV70_r, on = 'months'))-1)){ 
  NCAV70_q[i,1] <- quantile(NCAV70_r[i,], na.rm = TRUE, 0.1)
  NCAV70_q[i,2] <- quantile(NCAV70_r[i,], na.rm = TRUE, 0.9)}
NCAV70_q <- cbind(index(NCAV70_r),NCAV70_q)
names(NCAV70_q) <- c("Date","Q1","Q2")

NetIc70_q <- as.data.frame(1:(length(endpoints(NetIc70_r, on = 'months'))-1))
for (i in 1:(length(endpoints(NetIc70_r, on = 'months'))-1)){ 
  NetIc70_q[i,1] <- quantile(NetIc70_r[i,], na.rm = TRUE, 0.1)
  NetIc70_q[i,2] <- quantile(NetIc70_r[i,], na.rm = TRUE, 0.9)}
NetIc70_q <- cbind(index(NetIc70_r),NetIc70_q)
names(NetIc70_q) <- c("Date","Q1","Q2")

GPA70_q <- as.data.frame(1:(length(endpoints(GPA70, on = 'months'))-1))
for (i in 1:(length(endpoints(GPA70, on = 'months'))-1)){ 
  GPA70_q[i,1] <- quantile(GPA70[i,], na.rm = TRUE, 0.1)
  GPA70_q[i,2] <- quantile(GPA70[i,], na.rm = TRUE, 0.9)}
GPA70_q <- cbind(index(GPA70),GPA70_q)
names(GPA70_q) <- c("Date","Q1","Q2")
...
mktcap70_q <- as.data.frame(1:(length(endpoints(mktcap70, on = 'months'))-1))
for (i in 1:(length(endpoints(mktcap70, on = 'months'))-1)){ 
  mktcap70_q[i,1] <- quantile(mktcap70[i,], na.rm = TRUE, 0.5)}
mktcap70_q <- cbind(index(mktcap70),mktcap70_q)
names(mktcap70_q) <- c("Date","Q1")
...
high70_1 <- as.data.frame(ifelse(coredata(NCAV70) >= NCAV70_q$Q2, 1, NA))
low70_1 <- as.data.frame(ifelse(coredata(NCAV70) <= NCAV70_q$Q1, 1, NA))

high70_2 <- as.data.frame(ifelse(coredata(NetIc70) >= NetIc70_q$Q2, 1, NA))
low70_2 <- as.data.frame(ifelse(coredata(NetIc70) <= NetIc70_q$Q1, 1, NA))

high70_3 <- as.data.frame(ifelse(coredata(GPA70) >= GPA70_q$Q2, 1, NA))
low70_3 <- as.data.frame(ifelse(coredata(GPA70) <= GPA70_q$Q1, 1, NA))
...
hh12_70 <- high70_1*high70_2
lh12_70 <- low70_1*high70_2
hl12_70 <- high70_1*low70_2
ll12_70 <- low70_1*low70_2

hh13_70 <- high70_1*high70_3
lh13_70 <- low70_1*high70_3
hl13_70 <- high70_1*high70_3
ll13_70 <- low70_1*low70_3

hh23_70 <- high70_2*high70_3
lh23_70 <- low70_2*high70_3
hl23_70 <- high70_2*low70_3
ll23_70 <- low70_2*low70_3
...
hhh_70 <- high70_1*high70_2*high70_3
lhh_70 <- low70_1*high70_2*high70_3
hlh_70 <- high70_1*low70_2*high70_3
hhl_70 <- high70_1*high70_2*low70_3
llh_70 <- low70_1*low70_2*high70_3
lhl_70 <- low70_1*high70_2*low70_3
hll_70 <- high70_1*low70_2*low70_3
lll_70 <- low70_1*low70_2*low70_3
...
hh12_70 <- hh12_70 * coredata(mktcap70_jun)
lh12_70 <- lh12_70 * coredata(mktcap70_jun)
hl12_70 <- hl12_70 * coredata(mktcap70_jun)
ll12_70 <- ll12_70 * coredata(mktcap70_jun)

hh13_70 <- hh13_70 * coredata(mktcap70_jun)
lh13_70 <- lh13_70 * coredata(mktcap70_jun)
hl13_70 <- hl13_70 * coredata(mktcap70_jun)
ll13_70 <- ll13_70 * coredata(mktcap70_jun)

hh23_70 <- hh23_70 * coredata(mktcap70_jun)
lh23_70 <- lh23_70 * coredata(mktcap70_jun)
hl23_70 <- hl23_70 * coredata(mktcap70_jun)
ll23_70 <- ll23_70 * coredata(mktcap70_jun)
...
hh12_70 <- hh12_70/rowSums(hh12_70, na.rm=TRUE)
lh12_70 <- lh12_70/rowSums(lh12_70, na.rm=TRUE)
hl12_70 <- hl12_70/rowSums(hl12_70, na.rm=TRUE)
ll12_70 <- ll12_70/rowSums(ll12_70, na.rm=TRUE)

hh13_70 <- hh13_70/rowSums(hh13_70, na.rm=TRUE)
lh13_70 <- lh13_70/rowSums(lh13_70, na.rm=TRUE)
hl13_70 <- hl13_70/rowSums(hl13_70, na.rm=TRUE)
ll13_70 <- ll13_70/rowSums(ll13_70, na.rm=TRUE)

hh23_70 <- hh23_70/rowSums(hh23_70, na.rm=TRUE)
lh23_70 <- lh23_70/rowSums(lh23_70, na.rm=TRUE)
hl23_70 <- hl23_70/rowSums(hl23_70, na.rm=TRUE)
ll23_70 <- ll23_70/rowSums(ll23_70, na.rm=TRUE)
...
hhh_70 <- hhh_70 * coredata(mktcap70_jun)
lhh_70 <- lhh_70 * coredata(mktcap70_jun)
hlh_70 <- hlh_70 * coredata(mktcap70_jun)
hhl_70 <- hhl_70 * coredata(mktcap70_jun)
llh_70 <- llh_70 * coredata(mktcap70_jun)
lhl_70 <- lhl_70 * coredata(mktcap70_jun)
hll_70 <- hll_70 * coredata(mktcap70_jun)
lll_70 <- lll_70 * coredata(mktcap70_jun)
...
hhh_70 <- hhh_70/rowSums(hhh_70, na.rm=TRUE)
lhh_70 <- lhh_70/rowSums(lhh_70, na.rm=TRUE)
hlh_70 <- hlh_70/rowSums(hlh_70, na.rm=TRUE)
hhl_70 <- hhl_70/rowSums(hhl_70, na.rm=TRUE)
llh_70 <- llh_70/rowSums(llh_70, na.rm=TRUE)
lhl_70 <- lhl_70/rowSums(lhl_70, na.rm=TRUE)
hll_70 <- hll_70/rowSums(hll_70, na.rm=TRUE)
lll_70 <- lll_70/rowSums(lll_70, na.rm=TRUE)
...
hh12_70 <- xts(x = hh12_70, order.by = index(NCAV70))
lh12_70 <- xts(x = lh12_70, order.by = index(NCAV70))
hl12_70 <- xts(x = hl12_70, order.by = index(NCAV70))
ll12_70 <- xts(x = ll12_70, order.by = index(NCAV70))

hh13_70 <- xts(x = hh13_70, order.by = index(NCAV70))
lh13_70 <- xts(x = lh13_70, order.by = index(NCAV70))
hl13_70 <- xts(x = hl13_70, order.by = index(NCAV70))
ll13_70 <- xts(x = ll13_70, order.by = index(NCAV70))

hh23_70 <- xts(x = hh23_70, order.by = index(NCAV70))
lh23_70 <- xts(x = lh23_70, order.by = index(NCAV70))
hl23_70 <- xts(x = hl23_70, order.by = index(NCAV70))
ll23_70 <- xts(x = ll23_70, order.by = index(NCAV70))
...
hhh_70 <- xts(x = hhh_70, order.by = index(NCAV70))
lhh_70 <- xts(x = lhh_70, order.by = index(NCAV70))
hlh_70 <- xts(x = hlh_70, order.by = index(NCAV70))
hhl_70 <- xts(x = hhl_70, order.by = index(NCAV70))
llh_70 <- xts(x = llh_70, order.by = index(NCAV70))
lhl_70 <- xts(x = lhl_70, order.by = index(NCAV70))
hll_70 <- xts(x = hll_70, order.by = index(NCAV70))
lll_70 <- xts(x = lll_70, order.by = index(NCAV70))
...
hh12_70[is.na(hh12_70)] <- 0
lh12_70[is.na(lh12_70)] <- 0
hl12_70[is.na(hl12_70)] <- 0
ll12_70[is.na(ll12_70)] <- 0

hh13_70[is.na(hh13_70)] <- 0
lh13_70[is.na(lh13_70)] <- 0
hl13_70[is.na(hl13_70)] <- 0
ll13_70[is.na(ll13_70)] <- 0

hh23_70[is.na(hh23_70)] <- 0
lh23_70[is.na(lh23_70)] <- 0
hl23_70[is.na(hl23_70)] <- 0
ll23_70[is.na(ll23_70)] <- 0
...
hhh_70[is.na(hhh_70)] <- 0
lhh_70[is.na(lhh_70)] <- 0
hlh_70[is.na(hlh_70)] <- 0
hhl_70[is.na(hhl_70)] <- 0
llh_70[is.na(llh_70)] <- 0
lhl_70[is.na(lhl_70)] <- 0
hll_70[is.na(hll_70)] <- 0
lll_70[is.na(lll_70)] <- 0
...
portfolio_hh12_70 = Return.portfolio(ret70, hh12_70, verbose = TRUE)
portfolio_lh12_70 = Return.portfolio(ret70, lh12_70, verbose = TRUE)
portfolio_hl12_70 = Return.portfolio(ret70, hl12_70, verbose = TRUE)
portfolio_ll12_70 = Return.portfolio(ret70, ll12_70, verbose = TRUE)

portfolio_hh13_70 = Return.portfolio(ret70, hh13_70, verbose = TRUE)
portfolio_lh13_70 = Return.portfolio(ret70, lh13_70, verbose = TRUE)
portfolio_hl13_70 = Return.portfolio(ret70, hl13_70, verbose = TRUE)
portfolio_ll13_70 = Return.portfolio(ret70, ll13_70, verbose = TRUE)

portfolio_hh23_70 = Return.portfolio(ret70, hh23_70, verbose = TRUE)
portfolio_lh23_70 = Return.portfolio(ret70, lh23_70, verbose = TRUE)
portfolio_hl23_70 = Return.portfolio(ret70, hl23_70, verbose = TRUE)
portfolio_ll23_70 = Return.portfolio(ret70, ll23_70, verbose = TRUE)
...
portfolio_hhh_70 = Return.portfolio(ret70, hhh_70, verbose = TRUE)
portfolio_lhh_70 = Return.portfolio(ret70, lhh_70, verbose = TRUE)
portfolio_hlh_70 = Return.portfolio(ret70, hlh_70, verbose = TRUE)
portfolio_hhl_70 = Return.portfolio(ret70, hhl_70, verbose = TRUE)
portfolio_llh_70 = Return.portfolio(ret70, llh_70, verbose = TRUE)
portfolio_lhl_70 = Return.portfolio(ret70, lhl_70, verbose = TRUE)
portfolio_hll_70 = Return.portfolio(ret70, hll_70, verbose = TRUE)
portfolio_lll_70 = Return.portfolio(ret70, lll_70, verbose = TRUE)
...
HML1_70 <- portfolio_hh12_70$returns - portfolio_ll12_70$returns
HML2_70 <- portfolio_hh13_70$returns - portfolio_ll13_70$returns
HML3_70 <- portfolio_hh23_70$returns - portfolio_ll23_70$returns
...
HML4_70 <- portfolio_hhh_70$returns - portfolio_lll_70$returns
HML5_70 <- (portfolio_hhh_70$returns + portfolio_lhh_70$returns + portfolio_hlh_70$returns + portfolio_hhl_70$returns)/4 - (portfolio_lll_70$returns + portfolio_llh_70$returns + portfolio_lhl_70$returns + portfolio_hll_70$returns)/4
HML6_70 <- (portfolio_lhh_70$returns + portfolio_hlh_70$returns + portfolio_hhl_70$returns)/3 - (portfolio_llh_70$returns + portfolio_lhl_70$returns + portfolio_hll_70$returns)/3
...
charts.PerformanceSummary(HML1_70, main = "HML1 70years")
charts.PerformanceSummary(HML2_70, main = "HML2 70years")
charts.PerformanceSummary(HML3_70, main = "HML3 70years")
...
charts.PerformanceSummary(HML4_70, main = "HML4 70years")
charts.PerformanceSummary(HML5_70, main = "HML5 70years")
charts.PerformanceSummary(HML6_70, main = "HML6 70years")
...
ret_spy_70 = Return.calculate(prices)
ret_spy_70 = ret_spy_70['1950-07::2020-12']

colnames(ret_spy_70) <- 'SPY'
charts.PerformanceSummary(ret_spy_70, main = "S&P500")
...
factors2_70 <- cbind(HML1_70,HML2_70,HML3_70,ret_spy_70)
names(factors2_70) <- c("HML1","HML2","HML3","SPY")
charts.PerformanceSummary(factors2_70, main = "2 Factor Model Backtesting 70years")
...
factors3_70 <- cbind(HML4_70,HML5_70,HML6_70,ret_spy_70)
names(factors3_70) <- c("HML4","HML5","HML6","SPY")
charts.PerformanceSummary(factors3_70, main = "3 Factor Model Backtesting 70years")
...
result2_70 <- as.data.frame(c(1:2))
for (i in 1:4){
  test2_70 <- coeftest(lm(factors2_70[,i] ~ 1), vcov. = NeweyWest(lm(factors2_70[,i] ~ 1), lag=5, prewhite = F, adjust = T))
  result2_70[1,i] <- test2_70[,1]
  result2_70[2,i] <- test2_70[,3]}
names(result2_70) <- c("HML1_70","HML2_70","HML3_70","SPY_70")
rownames(result2_70) <- c("return", "t-stat")
...
result3_70 <- as.data.frame(c(1:2))
for (i in 1:4){
  test3_70 <- coeftest(lm(factors3_70[,i] ~ 1), vcov. = NeweyWest(lm(factors3_70[,i] ~ 1), lag=5, prewhite = F, adjust = T))
  result3_70[1,i] <- test3_70[,1]
  result3_70[2,i] <- test3_70[,3]}
names(result3_70) <- c("HML4_70","HML5_70","HML6_70","SPY_70")
rownames(result3_70) <- c("return", "t-stat")
